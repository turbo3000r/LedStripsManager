...
// Pin assignments for LED channels (Green, Yellow)
// Reduced to 2 channels as requested
constexpr int CHANNEL_PINS[2] = {26, 25}; // 26: Green, Blue; 25 Red, Yellow
constexpr int NUM_CHANNELS = 2;

// Zero-cross detection pin
constexpr int ZERO_CROSS_PIN = 27;
...
constexpr unsigned int MIN_DELAY_US = 500;      // Minimum safe delay (Increased from 100us to 500us for reliable latching)
...
// Zero-cross debounce filter (8ms)
// Increased from 3ms to 8ms to filter out ANY noise during the phase cycle
// Real zero-cross events occur every 10ms, so we only accept events in the 8-10ms window
constexpr unsigned long ZC_DEBOUNCE_US = 9900;  // 8ms debounce window
...
static constexpr const char* DEVICE_ID = "esp_livingroom_2";
...
static constexpr const char* MQTT_CLIENT_ID = "ledstrip_dimmer";

// MQTT Topics (fully configurable per device)
static constexpr const char* MQTT_TOPIC_SET_STATIC   = "lights/room1/esp_dimmer_2/set_static";
static constexpr const char* MQTT_TOPIC_SET_PLAN     = "lights/room1/esp_dimmer_2/set_plan";
static constexpr const char* MQTT_TOPIC_HEARTBEAT    = "lights/room1/esp_dimmer_2/heartbeat";
